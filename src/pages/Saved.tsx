...
export default function Saved() {
  ...
  const loadData = useCallback(async (activeUser: User) => {
    setLoading(true);
    try {
      const [savedResult, reminderResult, searchResult, deliveryResult] = await Promise.all([
      ...
      console.error(error);
      toast.error("Could not load your saved funding. Apply the latest database migration if this is a new deployment.");
  ...
  const savedGrants = useMemo(() => saved.filter((row) => savedRowToOpportunity(row).fundingType === "GRANT"), [saved]);
  const savedLoans = useMemo(() => saved.filter((row) => savedRowToOpportunity(row).fundingType === "LOAN"), [saved]);
  ...
  const renderSavedItems = (rows: SavedRow[]) => rows.length === 0 ? <EmptyState /> : <div className="grid gap-5 lg:grid-cols-2">{rows.map((row) => {
    const opportunity = savedRowToOpportunity(row);
    const reminder = remindersBySavedId.get(row.id);
    const detailUrl = opportunity.source === "grants.gov" ? `/federal-grant/${opportunity.externalId}` : opportunity.programId ? `/program/${opportunity.programId}` : null;
    return <Card key={row.id}><CardHeader><div className="flex items-start justify-between gap-3"><div><div className="mb-2 flex gap-2"><Badge>{opportunity.fundingType === "LOAN" ? "Loan" : "Grant"}</Badge><Badge variant="secondary">{opportunity.sourceName}</Badge>{reminder && <Badge variant="outline"><CalendarClock className="mr-1 h-3 w-3" />Reminder set</Badge>}</div><CardTitle>{row.title}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{row.sponsor}</p></div><Button size="icon" variant="ghost" onClick={() => removeSaved(row.id)} aria-label="Remove"><Trash2 className="h-4 w-4" /></Button></div></CardHeader><CardContent className="space-y-4">
      <div className="flex flex-wrap items-center gap-3"><Select value={row.pipeline_status} onValueChange={(value) => updateStatus(row.id, value)}><SelectTrigger className="w-44" aria-label="Tracking stage"><SelectValue /></SelectTrigger><SelectContent>{PIPELINE.map((status) => <SelectItem key={status} value={status}><span className="capitalize">{status}</span></SelectItem>)}</SelectContent></Select>{row.deadline && <span className="text-sm text-muted-foreground">Deadline {new Date(`${row.deadline}T12:00:00`).toLocaleDateString()}</span>}</div>
      ...
      <div className="flex gap-2">{detailUrl && <Button asChild variant="outline"><Link to={detailUrl} onClick={() => void recordOpportunityView(opportunity)}>Review details</Link></Button>}<Button asChild><a href={row.official_url} target="_blank" rel="noreferrer" onClick={() => void recordOpportunityView(opportunity)}>Official source <ExternalLink className="ml-2 h-4 w-4" /></a></Button></div>
    </CardContent></Card>;
  })}</div>;

  if (loading) return <DashboardLayout><div className="p-12 text-center text-muted-foreground">Loading your saved funding…</div></DashboardLayout>;

  return (
    <DashboardLayout>
      <main className="container mx-auto px-4 py-8">
        <header className="mb-8"><div className="flex items-center gap-3"><Bookmark className="h-8 w-8 text-primary" /><h1 className="text-4xl font-bold">Saved grants & loans</h1></div><p className="mt-2 text-lg text-muted-foreground">Keep promising funding in one place, add reminders, and update each item's progress.</p></header>
        ...
        <Tabs defaultValue="all" className="space-y-6">
          <TabsList><TabsTrigger value="all">All saved ({saved.length})</TabsTrigger><TabsTrigger value="grants">Grants ({savedGrants.length})</TabsTrigger><TabsTrigger value="loans">Loans ({savedLoans.length})</TabsTrigger><TabsTrigger value="alerts">Alerts ({searches.length + reminders.length})</TabsTrigger></TabsList>
          <TabsContent value="all">{renderSavedItems(saved)}</TabsContent>
          <TabsContent value="grants">{renderSavedItems(savedGrants)}</TabsContent>
          <TabsContent value="loans">{renderSavedItems(savedLoans)}</TabsContent>
...
function EmptyState() {
  return <div className="rounded-xl border border-dashed py-16 text-center"><Search className="mx-auto mb-4 h-12 w-12 text-muted-foreground" /><h2 className="text-xl font-semibold">Nothing saved here yet</h2><p className="mt-2 text-muted-foreground">Save grants or loans to compare them and track your next steps.</p><div className="mt-5 flex justify-center gap-3"><Button asChild><Link to="/grants">Find grants</Link></Button><Button asChild variant="outline"><Link to="/loans">Find loans</Link></Button></div></div>;
}
